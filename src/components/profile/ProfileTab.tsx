...
//   console.log("API URL from Context:", URL);
  ...
  const profileFetch = async () => {
    ...
    try {
      ...
      if(res.status === 200) {
        ...
        // Map API response to UI structure with fallbacks for missing data
        const mappedProfile = {
          name: result.data.name || 'User Name Not Set',
          email: result.data.email || 'email@example.com',
          phone: result.data.mobile || '9865432100',
          address: result.data.address ? 
            `${result.data.address.street || ''} ${result.data.address.city || ''} ${result.data.address.state || ''} ${result.data.address.country || ''} ${result.data.address.zipCode || ''}`.trim() || 'Address not provided'
